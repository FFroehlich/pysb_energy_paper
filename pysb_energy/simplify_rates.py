...
def fmt_rct(r, species_dict, fmt='plain'):
    ...
    def fmt_fct(x):
        ...
        return {'latex': "$%s$" % latex(x).replace("__", "").replace(" % ", ""),
               'plain': str(x).replace(" % ", "")}[fmt]

    r_schema = " + ".join(map(fmt_fct, r.reactants)) + arrow + "".join(map(fmt_fct, r.products))
    return [r.name.replace("_", "-"), r.bidirectional_reaction_location+ ":" + r_schema, "K_f: " + fmt_fct(r.fwd_rate), "K_r: "+fmt_fct(r.rev_rate), "K_d: " + fmt_fct(r.kd)]


def gen_table(model, fmt='rst'):
    ...
    m = model
    species_ids = [('__s%d' % idx, str(m.species[idx])) for idx in range(len(m.species))]
    species_dict_bidirectional = dict(map(reversed, species_ids))

    all_rates_tidy = pd.DataFrame(data=np.vstack([[model_name(m)] \
                                                   + fmt_rct(x, species_dict_bidirectional, fmt=fmt) \
                                                   for x in gen_rct_rates(m)]), \
                                columns=["model", "name", "schema", "fwd_rate", "rev_rate", "kd"])

    return np.array(sorted(species_dict_bidirectional.items(), key = lambda x:x[1])),  all_rates_tidy.sort_values(["schema"])
...
def gen_rct_rates(m):
    ...
    Returns:
        A list of ReactionTuples, which are namedtuple objects with the fields:
                ["bidirectional_reaction_location", "name", "reactants", "products", "fwd_rate", "rev_rate", "kd"]
    """
    ## from a model, create a generator of all reaction rates binned by forward, reverse, kd

    rct_subs = dict(zip(
        [sp.Symbol(e.name) for e in m.expressions],
        [e.expand_expr(expand_observables=True) for e in m.expressions]
    )) #\+ [(sp.Symbol('__s%d' % m.get_species_index(complex_pattern=species)), latex(species)) for species in m.species])
    ...
    rxn_tup = collections.namedtuple("reaction", ["bidirectional_reaction_location", "name", "reactants", "products", "fwd_rate", "rev_rate", "kd"])
    rxn_ids = zip(range(len(fwd_rxn.keys())), fwd_rxn.keys()) # key the reactions by the position in bidirectional reaction array

    def make_rct_tuple(rxn_id):

        rule_name = rxn_id[1][0]
        bidirectional_reaction_location = "R" + str(rxn_id[0])
        rxn_dict_id = rxn_id[1]
        try:
            rev_rate = extra_simple(rev_rxn[rxn_dict_id]["rate"].subs(rct_subs))
        except:
            rev_rate = 'Irreversible' # reverse rxn never happens


        fwd_rate = extra_simple(fwd_rxn[rxn_dict_id]["rate"].subs(rct_subs))
        ...
        reactants, products = [species(fwd_rxn[rxn_dict_id][s]) for s in ["reactants", "products"]]

        return rxn_tup(bidirectional_reaction_location, rule_name, reactants, products, fwd_rate, rev_rate, kd)
...
def table_to_printstream(species, table, fmt='rst'):
    ...
    table = table.sort_values("model")[["schema", "fwd_rate", "rev_rate", "kd"]]

    printstream_info = [tb.tabulate(np.expand_dims(t[1].transpose(), axis=1), tablefmt=fmt) for t in table.iterrows()]
    ...
    if fmt == 'latex':
        ...
        return "\n============================\n".join(["", model_name,
    "\n".join(["%s: %s" % (r[1], r[0]) for r in species])] + printstream_info) + "\n"
